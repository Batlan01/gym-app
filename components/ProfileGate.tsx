...
import { GUEST_PROFILE_ID, LS_ACTIVE_PROFILE, LS_PROFILES, onboardedKey } from "@/lib/profiles";
...
type Props = {
  ...
  /** ✅ ha választott / létrehozott profilt, ezt visszajelezzük a LoginPage-nek */
  onPicked?: (profileId: string) => void;
};

export function ProfileGate({ children, forceOpen, onClosed, onPicked }: Props) {
  ...
  const pick = React.useCallback(
    (id: string) => {
      setActiveProfileId(id); // ✅ ez ír LS-be is
      closeGate();
      onPicked?.(id);
    },
  ...
  const onCreate = React.useCallback(() => {
    ...
    setProfiles((prev) => [p, ...ensureGuest(prev).filter((x) => x.id !== p.id)]);

    // onboarded default: hiányzó kulcs = false → onboardingra fog menni
    lsSet(onboardedKey(p.id), false);

    setName("");
    pick(p.id);
  }, [name, setProfiles, pick]);

  const continueGuest = React.useCallback(() => {
    setProfiles((prev) => ensureGuest(prev));
    // Guestet most kihagyjuk onboardingból:
    lsSet(onboardedKey(GUEST_PROFILE_ID), true);
    pick(GUEST_PROFILE_ID);
  }, [setProfiles, pick]);
  ...
  const deleteProfile = React.useCallback(
    (id: string) => {
      ...
      const ok = window.confirm("Biztos törlöd ezt a profilt?");
      if (!ok) return;
      ...
      <div className="relative mx-auto w-full max-w-md px-4 pt-16 pb-24">
        <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.06] shadow-2xl shadow-black/60 backdrop-blur-xl">
          ...
          <div className="relative p-6">
            ...
            <div className="mt-1 text-sm text-white/55">Minden edzés a profilodhoz kerül.</div>
                  ...
                  <div key={p.id} className="group flex items-stretch gap-2 rounded-2xl border border-white/10 bg-white/5 p-2">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-white/40">{p.id === GUEST_PROFILE_ID ? "Guest" : ""}</span>
                      </div>
