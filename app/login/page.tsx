...
import {
  ...
  linkWithCredential, OAuthProvider,
  signOut, type User,
} from "firebase/auth";
...
export default function LoginPage() {
  ...
  const [linkPending, setLinkPending] = React.useState<{ googleCred: any; email: string } | null>(null);
  ...
  const doEmail = React.useCallback(async () => {
    ...
    try {
      if (linkPending) {
        // Összevonás: Google cred + email/jelszó → egy fiók
        const emailCred = await signInWithEmailAndPassword(auth, linkPending.email, pass);
        await linkWithCredential(emailCred.user, linkPending.googleCred);
        setLinkPending(null);
        activateFirebaseUser(emailCred.user);
        return;
      }
  ...
  }, [email, pass, tab, linkPending, activateFirebaseUser]);
  ...
  const doGoogle = React.useCallback(async () => {
    ...
    try {
      ...
      if (e?.code === "auth/account-exists-with-different-credential") {
        // Ugyanaz az email már létezik email/jelszóval → linking flow
        const googleCred = OAuthProvider.credentialFromError(e) ?? (e as any).credential;
        const emailFromError = e?.customData?.email as string | undefined;
        if (googleCred && emailFromError) {
          setLinkPending({ googleCred, email: emailFromError });
        ...
        <div className="w-full max-w-sm">
          ...
          {linkPending ? (
            <div className="rounded-3xl p-5"
              style={{background:"var(--bg-surface)",border:"1px solid rgba(251,191,36,0.4)"}}>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-lg">🔗</span>
                <div className="text-sm font-black" style={{color:"var(--text-primary)"}}>Fiókok összevonása</div>
              </div>
              <div className="text-xs mb-4 leading-relaxed" style={{color:"var(--text-muted)"}}>
                A <span style={{color:"var(--text-primary)",fontWeight:700}}>{linkPending.email}</span> email
                már regisztrálva van jelszóval. Add meg a jelszavadat — ezután Google-lel
                és email/jelszóval is beléphetsz, és minden adatod megmarad.
              </div>
              <input value={pass} onChange={e => setPass(e.target.value)}
                placeholder="Jelszó" type="password" autoComplete="current-password"
              ...
              <button onClick={doEmail} disabled={busy || pass.length < 6}
                className="w-full rounded-2xl py-3 text-sm font-black pressable mb-2"
                style={{background:"#FBBF24",color:"#000",opacity:busy||pass.length<6?0.5:1}}>
                {busy ? "..." : "🔗 Összevonás és belépés"}
              </button>
              <button onClick={() => { setLinkPending(null); setPass(""); setErr(null); }}
                className="w-full rounded-2xl py-2 text-xs pressable"
                    ...
                    <>
                      ...
                      <div className="space-y-2 mb-3">
                        <input value={email} onChange={e => setEmail(e.target.value)}
                          ...
                          style={{background:"var(--bg-card)",border:"1px solid var(--border-mid)",color:"var(--text-primary)"}} />
                        <input value={pass} onChange={e => setPass(e.target.value)}
                          placeholder="Jelszó (min. 6 karakter)" type="password"
                      ...
                      <button onClick={doEmail} disabled={busy || !email.trim() || pass.length < 6}
                        ...
                        style={{background:"var(--accent-primary)",color:"#000",
                          opacity:busy||!email.trim()||pass.length<6?0.5:1}}>
