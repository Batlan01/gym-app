...
function WeightChart({ data, emptyLabel }: { data: WeightEntry[]; emptyLabel: string }) {
  ...
  if (chartData.length < 2) return (
    <div className="flex h-20 items-center justify-center text-xs" style={{color:"var(--text-muted)"}}>{emptyLabel}</div>
  );
  return (
    <ResponsiveContainer width="100%" height={90}>
      <LineChart data={chartData} margin={{top:4,right:4,left:-28,bottom:0}}>
        <XAxis dataKey="date" tick={{fontSize:9,fill:"rgba(255,255,255,0.25)"}} tickLine={false} axisLine={false} />
        <Tooltip contentStyle={{background:"var(--bg-elevated)",border:"none",borderRadius:10,fontSize:12}}
          labelStyle={{color:"var(--text-secondary)"}} itemStyle={{color:"var(--accent-primary)"}}
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 rounded-2xl p-3 transition-all"
      style={unlocked ? { background: colors.bg, border: `1px solid ${colors.border}` }
        : { background:"var(--surface-0)", opacity: 0.35, filter: "grayscale(1)" }}>
...
function WeightModal({ current, onSave, onClose, labels }: {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center" style={{paddingBottom:"env(safe-area-inset-bottom)"}}>
      ...
      <div className="relative w-full max-w-md rounded-t-3xl p-6" style={{background:"var(--bg-elevated)"}}>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-base font-bold" style={{color:"var(--text-muted)"}}>kg</span>
        </div>
        <div className="mt-4 flex gap-2">
          <button onClick={onClose} className="flex-1 rounded-2xl py-3.5 text-sm font-bold pressable"
            style={{background:"var(--surface-2)",color:"var(--text-secondary)"}}>{labels.cancel}</button>
  ...
  return (
    ...
    <div className="flex-1 pb-32 animate-in">
      ...
      <div className="px-4 pt-10 pb-5">
        <div className="text-[10px] font-black tracking-widest mb-4" style={{color:"var(--text-muted)"}}>ARCX</div>
        <div className="flex items-center gap-4 mb-5">
          <button onClick={() => fileRef.current?.click()} className="relative shrink-0 pressable" disabled={avatarUploading}>
            ...
            <div className="absolute -bottom-1 -right-1 grid h-5 w-5 place-items-center rounded-lg text-[10px] font-black"
              style={{background:"var(--surface-3)",color:"#fff"}}>{avatarUploading ? "…" : "✎"}</div>
          ...
          <button onClick={() => router.push("/settings")}
            ...
            style={{background:"var(--surface-1)",color:"var(--text-secondary)"}}>⚙️</button>
        </div>
        <LevelBar xp={xp} />
        ...
        <div className="px-4 space-y-3">
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={s.label} className="rounded-2xl p-4 text-center" style={{background:"var(--surface-1)"}}>
                <div className="text-2xl font-black leading-none" style={{color:"var(--accent-primary)"}}>{s.value}</div>
                {s.unit && <div className="text-[9px] font-bold mt-0.5" style={{color:"var(--text-muted)"}}>{s.unit}</div>}
                <div className="text-[10px] font-semibold mt-1" style={{color:"var(--text-muted)"}}>{s.label}</div>
              </div>
            ))}
          </div>
          ...
          {topEx.length > 0 && (
            <div className="rounded-2xl overflow-hidden" style={{background:"var(--surface-1)"}}>
              <div className="px-4 py-3" style={{borderBottom:"1px solid var(--border-subtle)"}}>
                <div className="text-[9px] font-black tracking-widest" style={{color:"var(--text-muted)"}}>{t.profile.top_exercises}</div>
              </div>
              {topEx.map((e,i) => (
                <div key={e.name} className="flex items-center gap-3 px-4 py-3"
                  ...
                  <div className="text-xs font-bold" style={{color:"var(--text-muted)"}}>{fmtK(e.volume)} kg</div>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="rounded-2xl overflow-hidden" style={{background:"var(--surface-1)"}}>
            <div className="flex items-center justify-between px-4 py-3"
              ...
              <div className="text-[9px] font-black tracking-widest" style={{color:"var(--text-muted)"}}>{t.profile.weight}</div>
      ...
      {tab==="settings" && (
        <div className="px-4 space-y-3">
          ...
          <div className="rounded-2xl overflow-hidden mb-3" style={{background:"var(--surface-1)"}}>
          ...
          <div className="rounded-2xl overflow-hidden" style={{background:"var(--surface-1)"}}>
            <div className="px-4 py-3" style={{borderBottom:"1px solid var(--border-subtle)"}}>
              <div className="text-[9px] font-black tracking-widest" style={{color:"var(--text-muted)"}}>{t.profile.theme_label}</div>
            </div>
          ...
          <div className="rounded-2xl overflow-hidden" style={{background:"var(--surface-1)"}}>
            <div className="px-4 py-3" style={{borderBottom:"1px solid var(--border-subtle)"}}>
              <div className="text-[9px] font-black tracking-widest" style={{color:"var(--text-muted)"}}>{t.profile.lang_label}</div>
            </div>
            <div className="p-3">
          ...
          <button onClick={() => router.push("/settings")}
            ...
            <div>
              ...
              <div className="text-xs mt-0.5" style={{color:"var(--text-muted)"}}>{t.profile.tech_settings_sub}</div>
            </div>
        ...
        <div className="mx-auto w-full max-w-md px-4 pt-8 pb-10">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={f.label}>
                <div className="text-[9px] font-black tracking-widest mb-1.5" style={{color:"var(--text-muted)"}}>{f.label}</div>
            ...
            <div>
              <div className="text-[9px] font-black tracking-widest mb-2" style={{color:"var(--text-muted)"}}>{t.profile.edit_goal}</div>
              <div className="grid grid-cols-3 gap-2">
                {[{v:"lose",l:t.profile.goal_lose},{v:"maintain",l:t.profile.goal_maintain},{v:"gain",l:t.profile.goal_gain}].map(x => (
                  <button key={x.v} onClick={() => setEditGoal(x.v)}
                    ...
                    style={editGoal===x.v ? {background:"var(--accent-primary)",color:"#000"} : {background:"var(--surface-2)",color:"var(--text-secondary)"}}>
                    {x.l}
                  </button>
                ))}
              </div>
            ...
            <div>
              <div className="text-[9px] font-black tracking-widest mb-2" style={{color:"var(--text-muted)"}}>{t.profile.edit_level}</div>
              <div className="grid grid-cols-3 gap-2">
                {[{v:"beginner",l:t.profile.level_beginner},{v:"intermediate",l:t.profile.level_mid},{v:"advanced",l:t.profile.level_advanced}].map(x => (
                  <button key={x.v} onClick={() => setEditLevel(x.v)}
                    className="rounded-2xl py-3 text-xs font-black pressable"
                    style={editLevel===x.v ? {background:"var(--accent-primary)",color:"#000"} : {background:"var(--surface-2)",color:"var(--text-secondary)"}}>
                    {x.l}
                  </button>
                ))}
              </div>
