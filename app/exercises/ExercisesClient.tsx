    ...
    <main className="mx-auto max-w-md px-4 pt-5 pb-28">
      <header className="mb-4">
        ...
        <div className="mt-1 flex items-end justify-between gap-3">
          <h1 className="text-2xl font-bold text-white">{showTiles ? "Katalógus" : group}</h1>
        ...
        {!showTiles ? (
          <div className="mt-3">
            ...
            />
            <div className="mt-2 text-xs text-white/45">{filtered.length} találat</div>
          </div>
        ) : (
          <div className="mt-2 text-sm text-white/55">Válassz egy izomcsoportot / alkategóriát.</div>
        )}
      </header>
      ...
      {showTiles ? (
        <section className="grid grid-cols-2 gap-3">
          {groups.map((g) => (
            <Tile key={g} title={g} subtitle="Megnyitás" href={`/exercises?g=${encodeURIComponent(g)}`} />
          ))}
        </section>
      ) : (
        <section className="space-y-2">
          {filtered.length === 0 ? (
            ...
            filtered.map((ex: any) => (
              <div key={ex.id} className="rounded-3xl border border-white/10 bg-white/5 p-4">
                <div className="text-sm font-semibold text-white">{ex.name}</div>
                <div className="mt-1 text-xs text-white/45">{pickGroup(ex)}</div>
              </div>
            ))
          )}
        </section>
